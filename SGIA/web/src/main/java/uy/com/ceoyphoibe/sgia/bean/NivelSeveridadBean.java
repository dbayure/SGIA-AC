...
public class NivelSeveridadBean {
	...
	/**
	 * @param nivelSeveridadSeleccionado
	 *            the nivelSeveridadSeleccionado to set
	 */
	...
	/**
	 * @param filaPerfilTemp
	 *            the filaPerfilTemp to set
	 */
	...
	public void seleccionGrupoActuadores() {
		if (grupoTemp.getDeAvance().equals("S")) {
			Iterator<ActuadorAvance> itActuadores = grupoTemp
					.getActuadoresAvance().iterator();
			if (itActuadores.hasNext())
				posicionesDisponibles = itActuadores.next()
						.getListaPosiciones();
		}
	}
	...
	public void onCancelFila(RowEditEvent event) {
		FacesMessage msg = new FacesMessage(
				"Se canceló modificar perfil de activación de ",
				((FilaPerfilActivacion) event.getObject()).getGrupoActuadores()
						.getNombre());
		FacesContext.getCurrentInstance().addMessage(null, msg);
	}
	...
	/**
	 * @param posicionesDisponibles
	 *            the posicionesDisponibles to set
	 */
