...
/**
 * Clase utilizada para atender pedidos REST y devolver un recurso web de una Posicion para ser utilizado en la vista
 */
@Path("/posiciones")
@RequestScoped
public class PosicionResourceRESTService {
	...
	public List<Posicion> listAll() {
		...
		final List<Posicion> results = em
				.createQuery(
						"select c from posiciones c order by c.idActuadorAvance, c.idPosicion")
				.getResultList();
		return results;
	}
