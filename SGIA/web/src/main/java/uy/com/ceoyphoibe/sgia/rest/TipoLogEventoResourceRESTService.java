...
/**
 * Clase utilizada para atender pedidos REST y devolver un recurso web de un TipoLogEvento para ser utilizado en la vista
 */
@Path("/tipologeventos")
@RequestScoped
public class TipoLogEventoResourceRESTService {
	...
	public List<TipoLogEvento> listAll() {
		...
		final List<TipoLogEvento> results = em.createQuery(
				"select c from tipoLogEventos c order by c.idTipoLogEvento")
				.getResultList();
		return results;
	}
